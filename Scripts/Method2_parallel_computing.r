...
# configuring cores - using the parallel-package to avoiding manual config
plan(multisession, workers = parallel::detectCores()-1 )

# Assignment 2 - now with parallelisation
MTweedieTests <- function(N, M, sig) {
  results <- furrr::future_map_dbl(1:M, ~simTweedieTest(N))
